...
class ParserTests(unittest.TestCase):
    ...
    def test_gacha_payload_validators_distinguish_network_success_from_valid_data(self):
        valid_pool = {
            "pool": {
                "pool-1": {"id": "pool-1", "start": 1, "end": 2},
            }
        }
        self.assertTrue(GachaSource._valid_pool_info(valid_pool))
        self.assertFalse(GachaSource._valid_pool_info({}))
        self.assertTrue(GachaSource._valid_server_data({
            "gachaPoolClient": [{"gachaPoolId": "pool-1"}],
        }))
        self.assertFalse(GachaSource._valid_server_data({"gachaPoolClient": []}))
        characters = {f"char_{index}": {"name": f"干员{index}"} for index in range(100)}
        self.assertTrue(GachaSource._valid_character_table(characters))
        self.assertFalse(GachaSource._valid_character_table({}))
