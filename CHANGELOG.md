...
- **日报头图过滤。** 头图选择不再使用已经结束的活动，优先展示仍未结束的活动、卡池或长期活动图片。
- **强制刷新真正重渲染。** `/方舟日报刷新` 每次执行都会重新检查数据源并调用渲染器，跳过最终图片缓存；普通日报和定时日报继续使用缓存。
- **版本信息对齐。** README、插件元数据与 AstrBot 版本徽章统一到当前发布要求。

## 1.0.0 - 2026-09-11

### 修复

- **B站动态字体一致性。** 修复动态标题或正文以 `//` 开头时被误判为协议相对 URL、导致字体子集漏收字形并触发服务器字体回退的问题；协议相对图片地址仍会被正确排除。
- **动态模板字重对齐。** 将内嵌字体声明对齐到 Source Han Serif CN Medium，减少不同渲染服务的字重回退差异。

## 0.9.9 - 2026-08-31

### 修复

- **公开招募职业筛选。** 对齐当前游戏数据的职业枚举：`TANK`、`SUPPORT`、`SPECIAL` 分别正确映射为重装、辅助、特种干员；保留旧枚举兼容历史快照。公招结果现会展示这三类干员。
- **职业标签回归覆盖。** 新增从公招名单数据到筛选结果的八职业回归测试，防止游戏数据枚举变化导致某一职业静默丢失。
